...
async function selectData(tableName: TableNames, user_id: number = 0,  columnName:string = '*'): Promise<(SelectEmailData | SelectTimeData | SelectAreaData)[] | string | number> {
    try {
        if (user_id === 0) {
            let query = `SELECT ${columnName} FROM "${tableName}";`;
            const result = await db.result<SelectEmailData | SelectTimeData | SelectAreaData>(query);
